# def find_parens(s):
#     toret = {}
#     pstack = []

#     for i, c in enumerate(s):
#         if c == '(':
#             pstack.append(i)
#         elif c == ')':
#             if len(pstack) == 0:
#                     print("No matching closing parens at: " + str(i))
#             toret[pstack.pop()] = i

#     if len(pstack) > 0:
#         print("No matching opening parens at: " + str(pstack.pop()))

#     return toret

def find_parens(gene_tree,taxa):
    #print(gene_tree)
    idx = gene_tree.find(taxa)
    pstack = []
    #print(gene_tree[idx-1] + " " + gene_tree[idx+len(taxa)])
    saved_val = -1
    direction = 0
    if(gene_tree[idx+len(taxa)]==','):
        direction = -1
        #print(" Comma on the right and left element = " + gene_tree[idx-1])
        for i in range(idx,len(gene_tree)):
            if(gene_tree[i] == '('):
                pstack.append(gene_tree[i])
                #print("Got (, stack size after pushing "+ str(len(pstack)))
                #print("stack contents ->" + str(pstack))
            elif(gene_tree[i] == ')'):
                sz = len(pstack)
                if(sz==0):
                    #print("Found the culprit at idx = " + str(i) + " which is: " + gene_tree[i])
                    #saved_val = i
                    #S = S[:Index] + S[Index + 1:]
                    gene_tree = gene_tree[:i] + gene_tree[i+1:]
                    idx = gene_tree.find(taxa)
                    gene_tree = gene_tree[:(idx-1)] + gene_tree[idx:]
                    idx = gene_tree.find(taxa)
                    gene_tree = gene_tree[:idx+len(taxa)] + gene_tree[idx+len(taxa)+1:]
                    gene_tree = gene_tree.replace(taxa,'')
                    break
                else:
                    pstack.pop()
                    sz = len(pstack)
                    #print("Got ), stack size after popping "+ str(len(pstack)))
                    #print("stack contents ->" + str(pstack))

                    if(i == (len(gene_tree) - 2) and ((len(pstack)) == 0)):
                        #print("Found the culprit at idx = " + str(i) + " which is: " + gene_tree[i])
                        gene_tree = gene_tree[:i] + gene_tree[i+1:]
                        idx = gene_tree.find(taxa)
                        gene_tree = gene_tree[:(idx-1)] + gene_tree[idx:]
                        idx = gene_tree.find(taxa)
                        gene_tree = gene_tree[:idx+len(taxa)] + gene_tree[idx+len(taxa)+1:]
                        gene_tree = gene_tree.replace(taxa,'')
                        break



    elif(gene_tree[idx-1] == ','):
        #print("Starting idx = " + str(idx)+ " Comma on the left and right element = " + gene_tree[idx+len(taxa)])
        for i in range(idx,-1,-1):
            if(gene_tree[i] == ')'):
                pstack.append(gene_tree[i])
                #print("Got ), stack size after pushing "+ str(len(pstack)))
                #print("stack contents ->" + str(pstack))
            elif(gene_tree[i] == '('):
                sz = len(pstack)
                if(sz==0):
                    #print("Found the culprit at idx = " + str(i) + " which is: " + gene_tree[i])
                    saved_val = i
                    gene_tree = gene_tree[:i] + gene_tree[i+1:]
                    idx = gene_tree.find(taxa)
                    gene_tree = gene_tree[:(idx-1)] + gene_tree[idx:]
                    idx = gene_tree.find(taxa)
                    gene_tree = gene_tree[:idx+len(taxa)] + gene_tree[idx+len(taxa)+1:]
                    gene_tree = gene_tree.replace(taxa,'')
                    break

                else:
                    pstack.pop()
                    sz = len(pstack)
                    #print("Got (, stack size after popping "+ str(len(pstack)))
                    #print("stack contents ->" + str(pstack))

                    if(i == 0 and sz == 0):
                        #print("Found the culprit at idx = " + str(i) + " which is: " + gene_tree[i])
                        gene_tree = gene_tree[:i] + gene_tree[i+1:]
                        idx = gene_tree.find(taxa)
                        gene_tree = gene_tree[:(idx-1)] + gene_tree[idx:]
                        idx = gene_tree.find(taxa)
                        gene_tree = gene_tree[:idx+len(taxa)] + gene_tree[idx+len(taxa)+1:]
                        gene_tree = gene_tree.replace(taxa,'')
                        break

    print(gene_tree)
    return gene_tree

def taxa_list_Maker(gene_tree):
    taxa_map={}
    counter=0
    final_dict={}
    gene_tree = gene_tree.strip()
    length=len(gene_tree)
    
    for i in range (0,length):
        if(gene_tree[i]!='(' and gene_tree[i]!= ')' and gene_tree[i]!=',' and gene_tree[i]!=';'):
            print(gene_tree[i])
            taxa_map[counter]=gene_tree[i]

    return      


#gene_tree = "(11.11,((4.4,(1.1,(3.3,2.2))),(10.10,(((5.5,6.6),(8.8,7.7),9.9))));"
#gene_tree = find_parens(gene_tree,"11.11")
#gene_tree = find_parens(gene_tree,"9.9")

def main():
    #gene_tree = "(11.11,((4.4,(1.1,(3.3,2.2))),(10.10,(((5.5,6.6),(8.8,7.7),9.9))));"
    #gene_tree="(((((5.5,(6.6,(8.8,7.7))),9.9),10.10),(4.4,((3.3,2.2),1.1))),11.11);"
    inp_file = open("StrongILS_ALLGT", "r")

    f1 = inp_file.readlines()
    
    for gt in f1:

        print(gt)

    taxa_list_Maker()
    #var = input("How many taxa: ")
    #print("You entered: " + var)
    # n=int(var)
    # taxa_list=[]

    # ret_tree=gene_tree
    # print("Enter "+ str(var)+" taxa:")
    # print(gene_tree)
    # for i in range(0,n):
    #     taxa=input()
    #     ret_tree=find_parens(ret_tree,taxa);
    #     #taxa_list.append(taxa)
    # print(ret_tree)
  
if __name__== "__main__":
  main()


#find_parens(gene_tree,"11.11")


#find_parens(gene_tree,"")