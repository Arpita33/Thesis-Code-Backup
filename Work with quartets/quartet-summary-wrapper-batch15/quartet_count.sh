...
cat $1| xargs -I@ sh -c 'echo -n "@" >'$tmp'; /home/arpita/Documents/Work\ with\ quartets/quartet-summary-wrapper-batch15/triplets.soda2103 printQuartets '$tmp';'|sed 's/.*: //'| sed 's/^/\(\(/'| sed 's/$/\)\)\;/'| sed 's/ | /\),\(/'| sed 's/ /\,/g'
